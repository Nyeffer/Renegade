	...
	void Update() {
			// Mobile Port of Movement
				...
				if(Input.touches[0].deltaTime > 0) {
					Player.transform.Translate(0, moveSpeed * Input.touches[0].deltaTime, 0); // Update the Player's Position to move Forward
				// In case if the player touch it second
				if(Input.touches[1].deltaTime > 0) {
					Player.transform.Translate(0, moveSpeed * Input.touches[1].deltaTime, 0); // Update the Player's Position to move Forward
			}
		}
	}
}